        ...
        for ($entryID=0;$entryID<sizeof($process->entries);$entryID++)
        {
            ...
            for($condactID=0;$condactID<sizeof($entry->condacts); $condactID++)
            {
                ...
                else if ($condact->Opcode == BEEP_OPCODE)
                {
                    ...
                    if (($target=='ZX') || ($target=='ZX81') || ($target=='NEXTDAAD')) // Zx Spectrum interpreter expects BEEP parameters in opposite order
                    {
                        $tmp = $condact->Param1;
...
{
    return ($target == 'NEXTDAAD') || ($target == 'ZX81') || ($target == 'CPM') || ($target == 'HTML') || ($target == 'ZX') || ($target == 'CPC') ||  ($target == 'C64') ||  ($target == 'PCW') ||  ($target == 'MSX') ||  ($target == 'AMIGA') ||  ($target == 'PC') ||  ($target == 'ST') || ($target == 'MSX2') || ($target=='CP4');
}
...
function getMachineIDByTarget($target, $subtarget)
{
  ...
  if ($target=='NEXTDAAD') return 0x0C; // New target for the NextDAAD ZX Spectrum Next interpreter
...
function Syntax()
{
    ...
    echo("+ <target>: target machine, should be 'ZX', 'NEXTDAAD', 'CPC', 'C64', 'CP4', 'MSX', 'MSX2', 'ZX81', 'PCW', 'PC', 'CPM', 'ST', 'AMIGA' or 'HTML'. Just to clarify, CP4 stands for Commodore Plus/4\n");
   ...
   switch ($target)
    {
        ...
        case 'NEXTDAAD': $baseLength = 100; break;
...
if (($adventure->debugMode) && ($target!='ZX') && ($target!='CPC') && ($target!='NEXTDAAD'))
{
